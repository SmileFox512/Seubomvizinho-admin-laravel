...
<script>
    ...
    function buildOneItem(item){
        if (item.published)
            var visible = `<img src="{{url('img/iconyes.png')}}" height="20px">`;
        else
            var visible = `<img src="{{url('img/iconno.png')}}" height="20px">`;
        ...
        return `
            ...
            @endif
                ...
                <td>${visible}</td>
                <td><div class="q-font-bold q-color-second">${item.timeago}</div>${toFormatedDate(item.updated_at)}</td>
                <td>
