...
<script>
    ...
    function buildOneItem(item){
        ...
        var data = new Date(item.updated_at);
        var dataFormatada = ('0' + data.getDate()).slice(-2) + "/" + ('0' + data.getMonth()).slice(-2) + "/" + data.getFullYear()+" " + data.getHours()+":" + data.getMinutes()+":" + data.getSeconds(); 
        ...
        return `
            <tr>
                ...
                ${email}
                <td>${item.roleName}</td>
                <td><div class="q-font-bold q-color-second">${item.timeago}</div>${dataFormatada}</td>
                <td>
